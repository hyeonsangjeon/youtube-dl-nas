...
- Keep every nginx temporary directory inside the prepared runtime directory and use inherited stderr so Debian images start correctly with a non-root `PUID`/`PGID`.
...
- Restricted new source URLs to HTTP(S) public-network destinations by default, with DNS and bounded redirect checks plus an explicit `YDLNAS_ALLOW_PRIVATE_SOURCES=true` administrator opt-in for trusted internal sources.
