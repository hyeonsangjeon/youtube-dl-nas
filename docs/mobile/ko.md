...
즉시 실행 프로필은 한 번의 공유 흐름을 유지합니다. NAS가 URL을 큐에 넣고 Smart Preflight를 실행한 뒤 두 번째 확인 화면 없이 자동으로 시작합니다. 짧은 시작 유예 동안 대시보드에서 요청을 되돌릴 수 있고, 같은 미디어가 있으면 다시 다운로드하지 않고 **이미 NAS에 있음** 결과를 표시합니다.
